...
export default function SOS() {
  ...
  const handleSOS = async () => {
    ...
    setStatus(`Alerting ${h.name}...`);
    
    // Simulate API calls
    setTimeout(() => {
      setStatus("Ambulance Dispatched!");
      setTimeout(() => setShowTracker(true), 1000);
    }, 1500);
  };

  return (
    <Box sx={{ minHeight: "100vh", background: T.bg, p: 3, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      {!showTracker ? (
        <Box sx={{ maxWidth: 500, width: "100%", background: T.surface, p: 4, borderRadius: 6, boxShadow: "0 20px 50px rgba(0,0,0,0.1)", textAlign: "center", border: `1px solid ${T.border}` }}>
          <Box sx={{ width: 100, height: 100, borderRadius: "50%", background: "rgba(220,38,38,0.1)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px" }}>
            <Typography sx={{ fontSize: 40 }}>🆘</Typography>
          </Box>
          <Typography variant="h4" sx={{ fontWeight: 900, mb: 2, color: T.text }}>EMERGENCY SOS</Typography>
          <Typography sx={{ color: T.textSub, mb: 4 }}>This will alert the nearest hospital and send your location to emergency services.</Typography>
